...
async function runAggregate() {
    try{
        await mongoose.connect("mongodb://localhost:27017/merntraining");
            ...
            const sale = mongoose.sale || mongoose.model("Sale",saleSchema);
            ...
            const aggregationResult = await sale.aggregate([
                // function of $match filters the incoming documents first.
                {$match: {status:"demo-sale"}},

                // $group: creates groups by a specific condition & calculate total & average
                {$group:{
                    _id:"$category",
